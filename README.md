...
## Website, Source Code, and Grading Notes

Source code repository:
https://github.com/Ovler-Young/Browser-Based-Survival-Analysis-Studio

Website:
This project is intended to be graded from the published website generated by [.github/workflows/deploy-pages.yml](/d:/dev/bdsfinal/.github/workflows/deploy-pages.yml:1). The repository also includes an optional `shinyapps.io` deployment path through [scripts/deploy_shinyapps.R](/d:/dev/bdsfinal/scripts/deploy_shinyapps.R:1).

How to use the website:

1. Open the website and wait 10-20 seconds on the first visit while `webR` and package files download.
2. Start with the default `survival::lung` example. The app is preloaded so graders can immediately inspect outputs without uploading data.
3. Review the `Overview`, `Kaplan-Meier`, `Cox Model`, and `Diagnostics` tabs.
4. To test custom data, switch `Data source` to `Upload file`, upload a `.csv` or `.xlsx`, and map the required columns from the sidebar.
5. Use the download buttons to inspect the filtered data, plots, workbook, and reproducible analysis files.

Highlights:

- The website is browser-based, so it can be graded without a local R setup.
- The default example dataset makes the main results visible immediately.
- The workflow is separated into clear tabs for validation, Kaplan-Meier analysis, Cox modeling, and diagnostics.
- Reproducible outputs can be downloaded directly from the app.

Additional functions beyond the basic requirements:

- Built-in example datasets: `lung`, `veteran`, and `ovarian`.
- Stratified Kaplan-Meier curve with risk table.
- Median survival summary table with confidence intervals.
- Log-rank test table for the selected grouping variable.
- Multivariable Cox model with selectable predictors, categorical reference levels, optional `strata()` terms, and ties handling.
- Hazard-ratio forest plot.
- Proportional hazards diagnostics with `cox.zph()` p-values and Schoenfeld residual plots.
- Missing-data summary, event-frequency table, and filtering diagnostics.
- Downloadable filtered dataset, Excel workbook, plot images, reproducible `.R` script, and `.qmd` report.
- Optional local PDF report generation when the app is run with Quarto installed.

...
build_repro_script <- reactive({
  ...
  metadata_lines <- c(
    sprintf("# Generated from Survival Analysis Studio on %s", format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z")),
...
build_repro_qmd <- reactive({
  ...
  qmd_lines <- c(
    ...
    paste0("subtitle: ", yaml_quote("Generated by Survival Analysis Studio")),
    ...
    "# Overview",
    "",
    "This report captures the current analysis selections from Survival Analysis Studio and is ready to render with Quarto's Typst output.",
    "",
    "## Configuration",
    "",
